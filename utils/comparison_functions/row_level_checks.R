...
#   select(keys, values.x, values.y, tol) %>%
#   mutate(
#     values.x = format(values.x, digits = 9) %>% trimws(),
#     values.y = format(values.y, digits = 9) %>% trimws(),
...
row_level_checks_html(df1, df2, unique_keys = c("USUBJID", "ID", "PARAMCD"))
